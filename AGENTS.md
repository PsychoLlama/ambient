...
- `extern fn` is the pure host boundary (abilities are the effectful one). Declarations own signatures/docs in `.ab` source; the host binds implementations in a `NativeRegistry` keyed by (module path, name) and supplies each function's **stable UUID** — the content identity. A native object encodes exactly `(uuid, param_count)`; names never enter it, so renames re-key the binding (loud build error via `verify_native_contract`, both directions) but never move a hash. Never reuse or renumber a UUID; core's assignments (`natives/core/`, block `FFFFFFFF-FFFF-FFFF-FFFE-…`) are pinned by a golden test. Natives are pure (`Vec<Value> -> Value`): no VM handle, no store access, no ability channel.
