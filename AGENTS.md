...
- Parameterized traits (`From<T>`) key coherence and dispatch symbols on each trait argument's _head_ uuid; every argument must have a nominal head. The `From`/`Into` bridge (`S: Into<T>` iff `impl From<S> for T`) anchors on `TRAIT_FROM_UUID`/`TRAIT_INTO_UUID` and is sound only because `validate_reserved_trait` pins both to one dictionary shape — never change either shape alone.
