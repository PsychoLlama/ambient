...
- Every item has one fully-qualified identity (`<module>::<name>`, e.g. `math::gcd`; `::` is the sole path separator, `.` is value/field access), and the engine's resolve pass (`crates/ambient-engine/src/resolve.rs`) canonicalizes every cross-module reference to it before checking. The checker env, intrinsic tables, ability resolver, and linker key off `QualifiedName::resolution_key()` alone — never add a second lookup convention or a spelling-specific code path.
