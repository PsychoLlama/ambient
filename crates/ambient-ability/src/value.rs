...
pub use crate::containers::{MapValue, SetValue};
...
impl Value {
    ...
    /// Create a new suspended ability value from the method's compiled
    /// reference, so identity, default implementation, and the `never`
    /// unwind flag can never disagree with the declaration (see
    /// [`SuspendedAbility::from_method_ref`]).
    #[must_use]
    pub fn suspended_ability(method_ref: &crate::AbilityMethodRef, args: Vec<Value>) -> Self {
        Self::SuspendedAbility(Arc::new(SuspendedAbility::from_method_ref(
            method_ref, args,
        )))
    }
...
mod tests {
    ...
    fn test_suspended_ability() {
        let method_ref = crate::AbilityMethodRef {
            ability_id: AbilityId::from_bytes([7; 32]),
            ability_uuid: uuid::Uuid::from_u128(7),
            signature: ambient_core::SignatureHash::from_bytes([2; 32]),
            impl_fn: None,
            never: true,
        };
        let ability = SuspendedAbility::from_method_ref(&method_ref, vec![Value::Number(42.0)]);
        assert_eq!(ability.ability_id, method_ref.ability_id);
        assert_eq!(ability.method, method_ref.method_key());
        // The unwind flag travels from the reference: a host-built
        // suspended value cannot silently downgrade a never method to
        // capture semantics.
        assert!(ability.never);
