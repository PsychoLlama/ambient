//! Code completion for the Ambient language, frontend-neutral.
//!
//! One derivation of "what completes here", shared by every frontend that
//! offers completions: the LSP (which renders [`CompletionItem`]s into
//! `lsp_types` items) and the REPL's tab completion. The items are plain
//! data — label, kind, signature text, optional snippet-shaped insert text —
//! with no protocol types, so frontends stay thin renderers and can never
//! disagree about what completes.
//!
//! Covers:
//! - Keywords (fn, let, if, etc.) and built-in types (Number, String, Bool)
//! - Function names from the current module and locals in scope
//! - Ability names and methods
//! - Core library modules and functions (`core::…::`)
//! - Package module members (`utils::`), trait-impl member stubs, and
//!   dot-member completion on typed receivers
...
use core_paths::{core_module_final_segment_matches, get_core_path_completions};

/// The semantic kind of a completion, mapped by each frontend onto its own
/// presentation vocabulary (LSP `CompletionItemKind`, REPL styling, …).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum CompletionKind {
    Keyword,
    /// A type name (builtin types, type aliases, associated-type stubs).
    Type,
    /// An interface-like item: ability, trait, or ability set.
    Interface,
    Function,
    Constant,
    Struct,
    ...
    Field,
    Method,
    /// A plain value insertion (the generated `unique(…)` UUID).
    #[default]
    Value,
}

/// A frontend-neutral completion item.
///
/// `label` is what the user is completing (and what a plain-text frontend
/// inserts); `insert_text` carries a richer insertion when one exists —
/// snippet-shaped (`${1:param}` placeholders) when `insert_is_snippet` is
/// set, plain text otherwise.
#[derive(Debug, Clone, Default)]
pub struct CompletionItem {
    /// The completed name, shown in the list and matched against the prefix.
    pub label: String,
    /// The item's semantic kind.
    pub kind: CompletionKind,
    /// A one-line description (`fn f(x: Number): Number`, `keyword`, …).
    pub detail: Option<String>,
    /// The inline signature suffix shown right after the label
    /// (`(x: Number): Number`).
    pub signature: Option<String>,
    /// Trailing context shown after the signature (origin module,
    /// `parameter`, `implement Show::show`, …).
    pub description: Option<String>,
    /// The item's documentation text, if any.
    pub doc: Option<String>,
    /// Overrides the text the frontend filters against, when it differs
    /// from the label (`fn show` for a method stub).
    pub filter_text: Option<String>,
    /// Overrides the frontend's sort order, when relevance beats alphabet.
    pub sort_text: Option<String>,
    /// The text to insert instead of the label, if it differs.
    pub insert_text: Option<String>,
    /// Whether `insert_text` uses snippet placeholder syntax (`${1:x}`).
...
pub fn get_completions(
    ...
    if ctx.after_dot {
        ...
        {
            items.extend(members::get_dot_completions(
                source,
                dot_offset,
                ctx.word_prefix,
                module,
    ...
    if let (Some(module), Some(module_path), Some(registry)) = (module, module_path, registry) {
        let stubs = members::get_impl_member_completions(
            source,
            ctx.offset,
            ctx.word_prefix,
            module,
    ...
    if let Some(module) = module {
        ...
        items.extend(locals::get_local_completions(
            module,
            ctx.offset,
            ctx.word_prefix,
        ));
    ...
    exports
        ...
        .filter_map(|export| {
            ...
            let item_kind = match export.kind {
                ExportKind::Function => CompletionKind::Function,
                ExportKind::Const => CompletionKind::Constant,
                ExportKind::Struct => CompletionKind::Struct,
                ExportKind::TypeAlias => CompletionKind::Type,
                ExportKind::Enum => CompletionKind::Enum,
                ExportKind::EnumVariant => CompletionKind::EnumVariant,
            ...
            Some(CompletionItem {
                ...
                doc: export.doc.as_ref().map(ToString::to_string),
                ..Default::default()
            })
        })
        .collect()
...
mod tests {
    use super::*;
    use crate::platform_prelude_resolver;
    ...
    fn test_ability_method_completions() {
        ...
        assert!(print_item.insert_is_snippet);
    ...
    fn test_core_root_completions() {
        ...
        for expected in ["collections", "primitives", "option", "result", "time"] {
            assert!(
                items
                    .iter()
                    .any(|i| i.label == expected && i.kind == CompletionKind::Module),
