...
//! driven entirely by the registry — keeping the "frontends are renderers"
//! invariant, with no frontend-private model of core.
...
pub(super) fn core_module_final_segment_matches(name: &str) -> bool {
...
/// driven entirely by the registry (no frontend-private model of core):
///
...
pub(super) fn get_core_path_completions(scope: &str, prefix: &str) -> Vec<CompletionItem> {
    let registry = crate::core_platform_registry();
...
fn export_kind_completion(kind: ExportKind) -> Option<(CompletionKind, &'static str)> {
    Some(match kind {
        ExportKind::Function => (CompletionKind::Function, "function"),
        ExportKind::Const => (CompletionKind::Constant, "constant"),
        ExportKind::Struct => (CompletionKind::Struct, "struct"),
        ExportKind::TypeAlias => (CompletionKind::Type, "type"),
        ExportKind::Enum => (CompletionKind::Enum, "enum"),
        ExportKind::Trait => (CompletionKind::Interface, "trait"),
        ExportKind::Ability => (CompletionKind::Interface, "ability"),
        ExportKind::Set => (CompletionKind::Interface, "ability set"),
