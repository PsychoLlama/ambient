...
fn variant_pat_segments(src: &str) -> Vec<String> {
    ...
    match pat.kind {
        crate::cst::CstPatternKind::Variant { name, .. } => {
...
/// A pattern is legal iff it parses cleanly in a `match` arm through both the
/// fail-fast and recovering parsers (they must never disagree).
